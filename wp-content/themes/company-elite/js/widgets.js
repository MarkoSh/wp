...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
      	...
      	var CompanyEliteCustomThemeImage = wp.media.controller.Library.extend({
      		defaults : _.defaults({
        ...
        company_elite_file_frame = wp.media.frames.company_elite_file_frame = wp.media({
        	button: {
        		text: jQuery( this ).data( 'uploader_button_text' )
        	},
        	state : 'custom-theme-insert-image',
        	states : [
	        	new CompanyEliteCustomThemeImage()
        	],
        	multiple: false
        });

        // When an image is selected, run a callback.
        company_elite_file_frame.on( 'select', function() {

        	var state = company_elite_file_frame.state('custom-theme-insert-image');
